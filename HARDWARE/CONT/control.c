...
double get_value(double *mytarget,double *myactual)	
{
	double res = 0;
...
int get_speed(double delta,int *myspeed)	//注意气压单位要改成浮点型
{
	int res = 0;
	if(myspeed == NULL)
	{
	...
	else if(2<abs(delta)&& abs(delta)<=10)
	{
		*myspeed = 15;
	}
	else if(10<abs(delta)&& abs(delta)<=60)
	{
		*myspeed = 30;
	}
	else 
